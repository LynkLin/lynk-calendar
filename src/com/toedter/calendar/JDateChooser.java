...
public class JDateChooser extends JPanel implements ActionListener,
	 ...
	 */
	public JDateChooser() {
		this(null, null, "yyyy-MM-dd", null);
	}
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if (evt.getPropertyName().equals("day")) {
			if (popup.isVisible()) {
				...
				int rv = ((Integer)evt.getNewValue()).intValue();
				if (rv > 0) {
					setDate(jcalendar.getCalendar().getTime());
				} else if(rv == JCalendar.LEFT_BUTTON_CLICK) {
					if(leftButtonValue == null || "".equals(leftButtonValue.trim())) {
						setDate(jcalendar.getLeftButtonText());
					} else {
						setDate(leftButtonValue);
					}
				} else if(rv == JCalendar.RIGHT_BUTTON_CLICK) {
					if(rightButtonValue == null || "".equals(rightButtonValue.trim())) {
						setDate(jcalendar.getRightButtonText());
					} else {
						setDate(rightButtonValue);
					}
				} else {
					setDate(null);
				}
	...
	/**
	 * set right button value
	 * @param rightButtonValue
	 */
	...
	public void setDate(Object obj) {
		if(obj instanceof Date) {
			dateEditor.setDate((Date) obj);
			
		} else {
			((JTextFieldDateEditor) dateEditor).setText(obj.toString());
		}
		if (getParent() != null) {
			getParent().invalidate();
		}
	...
	/**
	 * Sets the font of all components.
	 * 
	 * @param font
	 *            the new font
	 */
	 ...
	 *            the new font
	 */
	public void setSubUIFont(Font font) {
		if (isInitialized) {
			jcalendar.setFont(font);
		}
	}
